...
LOOPS_CODE = """function drawLoop(t, index, count) {
  let alpha = map(index, 0, count - 1, 120, 255);
  stroke(59, 130, 246, alpha);
  beginShape();
...
Each loop uses a sine wave to modulate the radius, creating an organic pulsing effect. Strokes blend from light to full blue against a slate background.
...
- Brand palette: slate background with blue strokes
